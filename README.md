...
1. Open a new issue in [octodemo/demo-bootstrap](https://github.com/octodemo/demo-bootstrap) using the Create Demo Codespaces template.
...
3. Review directory structure, discussing how Codespaces has cloned the repo, all files here. Run the application by hitting `F5` or use `Run -> Start Debugging` menu. Go to `PORTS` section, show that the application is currently running locally on port 5000. 
